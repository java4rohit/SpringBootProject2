...
public class AddKeywordPlan {
	...
	private static final Logger LOG = LoggerFactory.getLogger(AddKeywordPlan.class);
	...
	public static KeywordPlanResource generateKeywordPlanResourcce(String customerId, String idea, Properties properties) {
		KeywordPlanResource resource = null;
		...
		try {
			resource = new AddKeywordPlan().runExample(googleAdsClient, params.customerId,idea);
			...
			// GoogleAdsException.
			LOG.error("Request ID %s failed due to GoogleAdsException. Underlying errors:%n", gae.getRequestId());
			int i = 0;
	...
	private KeywordPlanResource runExample(GoogleAdsClient googleAdsClient, Long customerId,String idea) {
		...
		createKeywordPlanAdGroupKeywords(googleAdsClient, customerId, planAdGroupResource,idea);
		createKeywordPlanCampaignKeywords(googleAdsClient, customerId, planCampaignResource);

		KeywordPlanResource keywordPlanResource1 = new KeywordPlanResource();

		keywordPlanResource1.setKeywordPlanResource(keywordPlanResource);
		keywordPlanResource1.setPlanAdGroupResource(planCampaignResource);
		keywordPlanResource1.setPlanCampaignResource(planAdGroupResource);

		return keywordPlanResource1;
	...
	private static String createKeywordPlan(GoogleAdsClient googleAdsClient, Long customerId) {
		...
		try (KeywordPlanServiceClient client = googleAdsClient.getLatestVersion().createKeywordPlanServiceClient()) {
			...
			LOG.info("Created keyword plan: %s%n", resourceName);
			return resourceName;
		}
	}
	...
	private static String createKeywordPlanCampaign(GoogleAdsClient googleAdsClient, Long customerId,
		...
		try (KeywordPlanCampaignServiceClient client = googleAdsClient.getLatestVersion()
			...
			// Displays the result.
			String resourceName = response.getResults(0).getResourceName();
			LOG.info("Created campaign for keyword plan: %s%n", resourceName);
			return resourceName;
		}
	}
	...
	private static String createKeywordPlanAdGroup(GoogleAdsClient googleAdsClient, Long customerId,
		...
		try (KeywordPlanAdGroupServiceClient client = googleAdsClient.getLatestVersion()
			...
			// Displays the result.
			String resourceName = response.getResults(0).getResourceName();
			LOG.info("Created ad group for keyword plan: " + resourceName);
			return resourceName;
		}
	}
	...
	private static void createKeywordPlanAdGroupKeywords(GoogleAdsClient googleAdsClient, Long customerId,
			String planAdGroupResource, String idea) {

		
	//	List<KeywordPlanAdGroupKeyword> kwI = new ArrayList<>();
			...
			KeywordPlanAdGroupKeyword keyword1 = KeywordPlanAdGroupKeyword.newBuilder()
					...
					.setMatchType(KeywordMatchType.PHRASE).setText(idea).build();
		
		

		List<KeywordPlanAdGroupKeywordOperation> operations = java.util.stream.Stream.of(keyword1)
...
//		            .collect(Collectors.toList());
		...
		try (KeywordPlanAdGroupKeywordServiceClient client = googleAdsClient.getLatestVersion()
			...
			for (MutateKeywordPlanAdGroupKeywordResult result : response.getResultsList()) {
				LOG.info("Created keyword for keyword plan: %s%n", result.getResourceName());
			}
		}
	}
	 ...
	 * @return
	 */
	private String createKeywordPlanCampaignKeywords(GoogleAdsClient googleAdsClient, Long customerId,
			String planCampaignResource) {
		...
		try (KeywordPlanCampaignKeywordServiceClient client = googleAdsClient.getLatestVersion()
			...
			// Displays the result.
			String resourceName = response.getResults(0).getResourceName();
			// System.out.printf("Created negative keyword for keyword plan: %s%n",
			// resourceName);
			return resourceName;
		}
	}
